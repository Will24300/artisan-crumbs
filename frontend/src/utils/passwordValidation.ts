...
export function validatePassword(password: string): PasswordValidationResult {
  ...
  const notCommon = pwd.length > 0 && !isListedWeak && !isRepeatingChar && !isSequentialNumbers;
  ...
  const checks = {
    ...
    hasLowercase,
    hasNumber,
    notCommon: pwd.length === 0 ? false : notCommon,
  };
  ...
  if (notCommon && pwd.length >= 10) score += 1;

  if (pwd.length === 0) {
    return { isValid: false, score: 0, checks };
  }
