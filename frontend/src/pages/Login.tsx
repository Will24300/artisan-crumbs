...
function Login() {
  ...
  const handleFacebookSuccess = async (creds: { accessToken?: string; email?: string; name?: string }) => {
    ...
    try {
      ...
      dispatch(setCredentials({ token: data.token, user: data.user }));
      toast.success(`Welcome, ${data.user.name}! Signed in with Facebook 🍰`);
      navigate(data.user.role === "admin" ? "/admin" : "/");
    } catch {
      setError("Unable to connect to the server for Facebook login.");
    } finally {
      setLoading(false);
    }
  };

  // Combined error from both the form and the Google hook
  ...
  return (
    <div className="h-screen bg-[#F9F9F8] dark:bg-[#0f0d0c] flex flex-col justify-between px-4 sm:px-6 md:px-10 lg:px-16 overflow-hidden transition-colors duration-300">
      {/*
        Hidden off-screen div: Google Identity Services renders its own official
        button inside here. We click it programmatically from googleAuth.signIn().
        This is what triggers the real Google account-picker popup.
      */}
      <div
        ...
        >
          ...
          <div className="w-full md:w-1/2 px-4 py-8 md:p-10 flex flex-col justify-center bg-white dark:bg-stone-900">
            ...
            <div className="grid grid-cols-2 gap-3">
              {/* Sign in with Google — triggers real GIS popup */}
              <button
                id="login-google"
                type="button"
              ...
              {/* Sign in with Facebook — existing modal flow, unchanged */}
              <button
                id="login-facebook"
                type="button"
                disabled={loading || googleAuth.loading}
