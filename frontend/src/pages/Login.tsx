...
function Login() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Greet user with toastify pop up
      toast.success(
        mode === "login"
  ...
  return (
    <div className="h-screen bg-[#F9F9F8] flex flex-col justify-between px-4 sm:px-6 md:px-10 lg:px-16 overflow-hidden">
      {/* ── TOP BAR (Matches Header in Mockup) ────────────────────────────────── */}
      <header className="w-full max-w-5xl mx-auto py-3.5 flex items-center justify-between border-b border-gray-100 px-2 flex-shrink-0">
        <Link to="/" className="flex items-center gap-3">
          <img
            src={iconImg}
            alt="Artisan Crumbs Logo"
            className="h-8 w-8 object-contain"
          />
          <span className="font-bold text-[19px] text-[#0F172A] font-sans">
            Artisan Crumbs
          </span>
        </Link>
      ...
      <main className="flex-1 flex items-center justify-center p-3 md:p-6 overflow-hidden">
        <div className="max-w-5xl w-full bg-white rounded-[32px] overflow-hidden shadow-[0_20px_60px_rgba(15,23,42,0.05)] border border-gray-100/80 flex flex-row h-[520px] max-h-[calc(100vh-120px)]">
          
          {/* Left Column: Sourdough Bread Image + Overlay */}
          <div 
            className="hidden md:block w-1/2 relative bg-cover bg-center h-full flex-shrink-0"
            style={{ 
              backgroundImage: `url('https://images.unsplash.com/photo-1549931319-a545dcf3bc73?q=80&w=1000&auto=format&fit=crop')` 
            }}
          >
            {/* Soft dark-warm gradient overlay */}
          ...
          {/* Right Column: Form */}
          <div className="w-full md:w-1/2 p-6 md:p-10 flex flex-col justify-center bg-white h-full overflow-y-auto custom-scrollbar">
            <div className="mb-5">
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  ...
                  {mode === "login" && (
                    <button
                      type="button"
                      className="text-xs font-bold text-[#F59E0B] hover:text-[#D97706] hover:underline"
              ...
              {mode === "login" ? (
                <>
                  ...
                  <button
                    ...
                    className="font-bold text-[#F59E0B] hover:text-[#D97706] hover:underline"
                  >
                    Create an account
                  </button>
                </>
      ...
      {/* ── FOOTER BAR (Matches Footer in Mockup) ────────────────────────────── */}
